...
export default function Register() {
  ...
  return (
    <AuthLayout mode="register" title="Créez votre compte" description="Démarrez vos réunions en moins de 2 minutes.">
      ...
      <form onSubmit={handleSubmit} className="mt-7 flex flex-col gap-5 rounded-2xl border border-liseret bg-white p-6 shadow-[0_18px_50px_rgba(16,24,40,0.07)]">
        <Input label="Nom complet" name="name" autoComplete="name" autoFocus required minLength={2} maxLength={120} disabled={loading} value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex. Paul Biya" />
        ...
        <div className="flex flex-col gap-3">
          <PasswordField label="Mot de passe" name="password" autoComplete="new-password" required minLength={8} disabled={loading} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Minimum 8 caractères" />
        ...
        <Button type="submit" size="lg" loading={loading} className="mt-1 w-full">
          {loading ? 'Création en cours…' : 'Créer mon compte et démarrer'}
        </Button>
      </form>
    </AuthLayout>
  );
