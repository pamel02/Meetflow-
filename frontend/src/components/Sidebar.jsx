    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex w-[272px] shrink-0 flex-col border-r border-liseret bg-white transition-transform duration-200 ease-out md:static md:z-auto md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {['admin', 'organizer'].includes(user?.organization_role) && <div className="flex flex-col gap-2 px-4 pb-4 pt-1">
          <button type="button" onClick={handleQuickMeeting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-bordeaux-700 px-4 py-3 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(38,59,216,0.18)] transition hover:bg-bordeaux-800 hover:-translate-y-0.5">
            <span className="text-base leading-none">⚡</span>Réunion Rapide
          </button>
          <NavLink to="/app?nouvelle=1" onClick={closeSidebar} className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-liseret bg-white px-3 py-2 text-xs font-semibold text-encre-douce transition hover:border-bordeaux-400 hover:text-bordeaux-700">
